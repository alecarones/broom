...
def ilc(config: Configs, input_alms, compsep_run, nsim=None):
    ...
    output_maps = _ilc(config, obj_to_array(input_alms), compsep_run)
...
def _ilc_needlet(config: Configs, input_alms, compsep_run):
    ...
    for j in range(b_ell.shape[0]):
        output_alms += _ilc_needlet_j(config, input_alms, compsep_run, b_ell[j], j)
...
def _ilc_needlet_j(config: Configs, input_alms, compsep_run, b_ell, nl_scale):
    if "mask" in compsep_run:
