...
def pilc(config: Configs, input_alms, compsep_run, nsim=None):
    if compsep_run["method"] == "cpilc" or compsep_run["method"] == "c_pilc":
        compsep_run = _standardize_cilc(compsep_run, config.lmax)

    output_maps = _pilc(config, obj_to_array(input_alms), compsep_run)
...
def _pilc(config: Configs, input_alms, compsep_run):
    ...
    if compsep_run["domain"] == "pixel":
        output_maps = _pilc_pixel(config, input_alms, compsep_run)
    elif compsep_run["domain"] == "needlet":
        output_maps = _pilc_needlet(config, input_alms, compsep_run)

    del compsep_run["field"]

    return output_maps

def _pilc_needlet(config: Configs, input_alms, compsep_run):
    ...
    if ((config.field_out in ["QU", "QU_E", "QU_B"]) and config.pixel_window_out) or (config.field_out in ["E","B"]) or (config.field_out=="EB"):
        for c in range(output_maps.shape[-1]):
            if ("mask" in compsep_run) and (config.mask_type == "observed_patch") and ("_purify" in config.leakage_correction):
                ...
                alm_out = hp.map2alm(np.array([0.*output_maps[0,:,c],output_maps[0,:,c],output_maps[1,:,c]]) * compsep_run["mask"],lmax=config.lmax, pol=True)
            else:
                alm_out = hp.map2alm([0.*output_maps[0,:,c],output_maps[0,:,c],output_maps[1,:,c]],lmax=config.lmax, pol=True)
...
def _pilc_pixel(config: Configs, input_alms, compsep_run):
...
def _pilc_maps(config: Configs, input_maps, compsep_run, b_ell, nl_scale=None):
    ...
    if compsep_run["method"] == "cpilc":
        if nl_scale is None:
            compsep_run["A"] = _get_moments_SED(np.array(config.instrument.frequency)[good_channels_nl], compsep_run["constraints"]["moments"], beta_d=compsep_run["constraints"]["beta_d"], T_d=compsep_run["constraints"]["T_d"], beta_s=compsep_run["constraints"]["beta_s"], units=config.units, bandwidths=config.instrument.bandwidths[good_channels_nl] if config.bandpass_integrate else None)
            compsep_run["e"] = np.array(compsep_run["constraints"]["deprojection"])
        else:
            compsep_run["A"] = _get_moments_SED(np.array(config.instrument.frequency)[good_channels_nl], compsep_run["constraints"]["moments"][nl_scale], beta_d=compsep_run["constraints"]["beta_d"][nl_scale], T_d=compsep_run["constraints"]["T_d"][nl_scale], beta_s=compsep_run["constraints"]["beta_s"][nl_scale], units=config.units, bandwidths=config.instrument.bandwidths[good_channels_nl] if config.bandpass_integrate else None)
    ...
    if compsep_run["save_weights"]:
        ...
        filename = os.path.join(path_w, f"weights_{compsep_run['field']}")
        if nl_scale is not None:
            filename += f"_nl{nl_scale}"
        np.save(filename, w_ilc)
...
def _pilc_needlet_j_old(config: Configs, input_alms, compsep_run, b_ell, nl_scale):
...
#        else:
    ...
    for c in range(input_alms.shape[-1]):
        if ("mask" in compsep_run) and (config.mask_type == "observed_patch") and ("_purify" in config.leakage_correction):
            ...
            alm_out = hp.map2alm(np.concatenate([(0.*output_maps_nl[0,:,c])[np.newaxis],output_maps_nl[...,c]], axis=0) * compsep_run["mask"], lmax=lmax_, pol=True)[1:]
        else:
            alm_out = hp.map2alm(np.concatenate([(0.*output_maps_nl[0,:,c])[np.newaxis],output_maps_nl[...,c]], axis=0), lmax=lmax_, pol=True)[1:]
