...
def gilc(config: Configs, input_alms, compsep_run, nsim=None):
    ...
    output_maps = _gilc(config, obj_to_array(input_alms), nuis_alms, compsep_run)
...
def _gilc_post_processing(config: Configs, output_maps, compsep_run):
    if (output_maps.ndim==4) and ((output_maps.shape[1]==2 and config.field_out == "QU") or (output_maps.shape[1]==3 and config.field_out == "TQU")):
        ...
        for f, c in np.ndindex(output_maps.shape[0],output_maps.shape[-1]):
            outputs[f,...,c] = _EB_to_QU(output_maps[f,...,c],config.lmax)
        ...
        for f, c in np.ndindex(output_maps.shape[0],output_maps.shape[-1]):
            if config.field_out == "QU_E":
                output[f,...,c] = _E_to_QU(output_maps[f,:,c],config.lmax)
            elif config.field_out=="QU_B":
                output[f,...,c] = _B_to_QU(output_maps[f,:,c],config.lmax)
        if hasattr(compsep_run, "mask"):
...
def get_gnilc_maps(config: Configs, path_gnilc, nsim=None):
    ...
    if config.field_in in ["TQU", "TEB"]:
        if config.field_out == "T":
            ...
            gnilc_fields = (0,1,2)
    elif config.field_in in ["QU","EB"]:
        gnilc_fields = (0,1)
    elif config.field_in in ["T","E","B"]:
        gnilc_fields = 0
    ...
    if not os.path.exists(os.path.join(path_gnilc, "output_total")):
        ...
        for f, freq in enumerate(config.instrument.frequency):
            if nsim is None:
                filename = f"{path_gnilc}/output_total/{config.field_in}_output_total_{config.instrument.channels_tags[f]}_{config.fwhm_out}acm_ns{config.nside}.fits"
            else:
                filename = f"{path_gnilc}/output_total/{config.field_in}_output_total_{config.instrument.channels_tags[f]}_{config.fwhm_out}acm_ns{config.nside}_{nsim}.fits"
    ...
    if not os.path.exists(os.path.join(path_gnilc, "noise_residuals")):
        ...
        for f, freq in enumerate(config.instrument.frequency):
            if nsim is None:
                filename = f"{path_gnilc}/noise_residuals/{config.field_in}_noise_residuals_{config.instrument.channels_tags[f]}_{config.fwhm_out}acm_ns{config.nside}.fits"
            else:
                filename = f"{path_gnilc}/noise_residuals/{config.field_in}_noise_residuals_{config.instrument.channels_tags[f]}_{config.fwhm_out}acm_ns{config.nside}_{nsim}.fits"
    ...
    if os.path.exists(os.path.join(path_gnilc, "fgds_residuals")):
        ...
        for f, freq in enumerate(config.instrument.frequency):
            if nsim is None:
                filename = f"{path_gnilc}/fgds_residuals/{config.field_in}_fgds_residuals_{config.instrument.channels_tags[f]}_{config.fwhm_out}acm_ns{config.nside}.fits"
            else:
                filename = f"{path_gnilc}/fgds_residuals/{config.field_in}_fgds_residuals_{config.instrument.channels_tags[f]}_{config.fwhm_out}acm_ns{config.nside}_{nsim}.fits"
...
def _gilc(config: Configs, input_alms, nuis_alms, compsep_run):
    if input_alms.ndim == 4:
        ...
        for i in range(input_alms.shape[1]):
            output_maps[:,i] = _gilc_scalar(config, input_alms[:, i], nuis_alms[:, i], compsep_run)
    elif input_alms.ndim == 3:
        output_maps = _gilc_scalar(config, input_alms, nuis_alms, compsep_run)
    return output_maps
...
def _gilc_scalar(config: Configs, input_alms, nuis_alms, compsep_run):
    if compsep_run["domain"] == "pixel":
        output_maps = _gilc_pixel(config, input_alms, nuis_alms, compsep_run)
    elif compsep_run["domain"] == "needlet":
        output_maps = _gilc_needlet(config, input_alms, nuis_alms, compsep_run)
    return output_maps

def _gilc_pixel(config: Configs, input_alms, nuis_alms, compsep_run):
...
#            nuis_maps[n] = (hp.alm2map(np.ascontiguousarray(nuis_alms[n]), config.nside, lmax=config.lmax, pol=False) * compsep_run["mask"]) 
#        else:
    ...
    if config.pixel_window_out:
        for f, c in np.ndindex(output_maps.shape[0],output_maps.shape[-1]):
            alm_out = hp.map2alm(output_maps[f,:,c],lmax=config.lmax, pol=False)
            output_maps[f,:,c] = hp.alm2map(alm_out, config.nside, lmax=config.lmax, pol=False, pixwin=True)
    if "mask" in compsep_run:
        output_maps[:,compsep_run["mask"] == 0.,:] = 0.
    return output_maps

def _gilc_needlet(config: Configs, input_alms, nuis_alms, compsep_run):
    ...
    for j in range(b_ell.shape[0]):
        output_alms += _gilc_needlet_j(config, input_alms, nuis_alms, compsep_run, b_ell[j])
...
def _gilc_needlet_j(config: Configs, input_alms, nuis_alms, compsep_run, b_ell, depro_cmb=None, m_bias=0):
    if "mask" in compsep_run:
...
#    good_channels_nl = _get_good_channels_nl(config, b_ell)
    ...
    nuis_maps_nl = np.zeros((good_channels_nl.shape[0], 12 * nside_**2))
...
#        else:
    ...
    output_alms_j = np.zeros((input_alms.shape[0], hp.Alm.getsize(config.lmax), output_maps_nl.shape[-1]), dtype=complex)
    for n, channel in enumerate(good_channels_nl):
        output_alms_nl = np.array([hp.map2alm(output_maps_nl[n,:,c], lmax=lmax_, pol=False) for c in range(output_maps_nl.shape[-1])]).T
        if compsep_run["b_squared"]:
