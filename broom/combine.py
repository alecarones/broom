...
def _combine_(
    ...
    **kwargs
    ...
    """
    Combine input alms with component separation weights as defined in compsep_run dictionary and return/save the outputs.
    ...
    ----------
        config : Configs
            ...
            - fwhm_out: The full width at half maximum at which component separation has been performed (in arcminutes).
            - pixel_window_out: Boolean indicating whether to apply pixel windowing to the output maps.
            - save_compsep_products: Boolean indicating whether to save the combination outputs.
            - return_compsep_products: Boolean indicating whether to return the combination outputs.
        ...
        input_alms : np.ndarray
            Input alms to be combined with weights. The shape should be (n_freqs, n_fields, n_alms, n_comps) where:
            ...
            - n_comps: Number of components (e.g. total, noise, etc.).
        
        compsep_run : dict
            Dictionary containing the combination run parameters. It should include:
    ...
    -------
        SimpleNamespace or None
            Result of the combination of input alms with component separation weights.
            It will have all the attributes of input_alms, but each attribute will contain the combined maps.
    ...
    outputs = _combine_alms(config, obj_to_array(input_alms), compsep_run, **kwargs)
    
    if "pilc" not in compsep_run["compsep_path"] and "prilc" not in compsep_run["compsep_path"]:
        outputs = _ilc_post_processing(config, outputs, compsep_run, **kwargs)

    outputs = array_to_obj(outputs, input_attrs)
...
def _combine_alms(
    ...
    **kwargs
    ...
    """
    Combines input alms with component separation weights as defined in compsep_run dictionary.

    Parameters
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.
        input_alms : np.ndarray
            Input multifrequency set of alms to be combined. The shape should be (n_freqs, n_fields, n_alms, n_comps).
            n_fields is not None only if multiple fields are being processed simultaneously.
        compsep_run : dict
            Dictionary containing the combination run parameters. See '_combine_' for details.
        kwargs : dict
            Additional keywords for healpy map2alm function.
    ...
    -------
        np.ndarray
            Output maps resulting from the combination with component separation weights.
            The shape will be (n_fields, n_pixels, n_comps) with n_fields depending on config.field_out.
    """

    if "pilc" in compsep_run["compsep_path"] or "prilc" in compsep_run["compsep_path"]:
        if input_alms.ndim == 4:
            if input_alms.shape[1] == 3:
                raise ValueError("Polarization ILC does not support TEB alms, only EB, E or B alms.")
        ...
        output_maps = _combine_P(config, input_alms, compsep_run, **kwargs)
    else:
        if input_alms.ndim == 4:
        ...
        if input_alms.ndim == 4:
            if compsep_run["method"] == "ilc":
                ...
                output_maps = np.zeros((len(compsep_run["channels_out"]), input_alms.shape[1], 12 * config.nside**2, input_alms.shape[3]))
                
            for i in range(input_alms.shape[1]):
                compsep_run["field"] = fields_ilc[i]
                if compsep_run["method"] == "ilc":
                    output_maps[i] = _combine_scalar(config, input_alms[:, i], compsep_run, **kwargs)
                else:
                    output_maps[:, i] = _combine_scalar(config, input_alms[:, i], compsep_run, **kwargs)

        elif input_alms.ndim == 3:
            compsep_run["field"] = fields_ilc[0]
            output_maps = _combine_scalar(config, input_alms, compsep_run, **kwargs)
...
def _combine_scalar(
    ...
    **kwargs: Any
    ...
    """
    Combine input alms for a single scalar field with corresponding component separation weights as defined in compsep_run dictionary.
    ...
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.
        input_alms : np.ndarray
            Input multifrequency set of alms for a single scalar field.
    ...
    -------
        np.ndarray
        Outputs for single scalar field combination (in pixel or needlet domain) with corresponding component separation weights.
        The shape will be (n_pixels, n_comps).
    """

    domain = compsep_run["compsep_path"].split('ilc_')[1].split('_bias')[0]
    if domain == "pixel":
        return _combine_pixel(config, input_alms, compsep_run, **kwargs)
    elif domain == "needlet":
        return _combine_needlet(config, input_alms, compsep_run, **kwargs)
    else:
        raise ValueError(f"Domain {domain} not supported for propagation through selected component separation routine.")
...
def _combine_P(
    ...
    **kwargs: Any
    ...
    """
    Combine input multifrequency polarization alms with component separation P(R)ILC weights.
    ...
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.  
        input_alms : np.ndarray
            Input alms for polarization field
            The shape should be (n_freqs, (n_fields), n_alms, n_comps), with n_fields not None if both E and B fields are present.
        compsep_run : dict
            Dictionary containing the residuals estimates run parameters. See '_combine_' for details.
        kwargs : dict
            Additional keywords for healpy map2alm function.
    ...
    -------
        np.ndarray
            Output maps resulting from propagation through P(R)ILC. Shape is (n_fields, n_pixels, n_comps) where n_fields depends on config.field_out.
    """
    if "pilc_" in compsep_run["compsep_path"]:
        domain = compsep_run["compsep_path"].split('pilc_')[1].split('_bias')[0]
    elif "prilc_" in compsep_run["compsep_path"]:
        domain = compsep_run["compsep_path"].split('prilc_')[1].split('_bias')[0]

    if domain == "pixel":
        return _combine_P_pixel(config, input_alms, compsep_run, **kwargs)
    elif domain == "needlet":
        return _combine_P_needlet(config, input_alms, compsep_run, **kwargs)
    else:
        raise ValueError(f"Domain {domain} not supported for propagation with P(R)ILC weights.")

def _combine_needlet(
    config: Configs,
    input_alms: np.ndarray,
    compsep_run: dict,
    **kwargs: Any
    ...
    """
    Propagation of input multifrequency set of alms with component separation weights in needlet domain for a single scalar field.
    ...
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.  
        input_alms : np.ndarray
            Input alms to be progated through needlet compsep weights for a single scalar field.
            The shape should be (n_freqs, n_alms, n_comps).
        compsep_run : dict
            Dictionary containing the residuals estimates run parameters. See '_combine_' for details.
        kwargs : dict
    ...
    -------
        np.ndarray
        Output maps resulting from propagation through needlet component separation weights.
...
def _combine_needlet_j(
    ...
    **kwargs: Any
    ...
    """
    Combine input multifrequency set of alms with component separation weights for a single needlet band.
    ...
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.  
        input_alms : np.ndarray
            Input alms to be combined with needlet compsep weights for a single scalar field.
            The shape should be (n_freqs, n_alms, n_comps).
        compsep_run : dict
            Dictionary containing the residuals estimates run parameters. See '_combine_' for details.
        b_ell : np.ndarray
    ...
    -------
        np.ndarray
            Output alms resulting from combination with needlet component separation weights for a single band.
            Shape is (n_alms, n_comps).
    """

    if "cosine" in compsep_run["compsep_path"]:
        needlet_windows = "cosine"
    elif "mexican" in compsep_run["compsep_path"]:
        needlet_windows = "mexican"
    else:
        needlet_windows = "standard"
...
#        nside_, lmax_ = _get_nside_lmax_from_b_ell(b_ell,config.nside,config.lmax)
    if compsep_run["adapt_nside"]:
        nside_, lmax_ = _get_nside_lmax_from_b_ell(b_ell,config.nside,config.lmax,needlet_windows)
    else:
        nside_, lmax_ = config.nside, config.lmax
    ...
    if w_.ndim==1:
        ...
        elif compsep_run["method"] == "gilc":
            output_maps_nl_ = np.einsum('li,ijk->ljk', w_, input_maps_nl)
    elif w_.ndim==3:
        if compsep_run["method"] == "gilc":
            output_maps_nl_ = np.einsum('jli,ijk->ljk', w_, input_maps_nl)

    if compsep_run["method"] == "gilc":
        output_maps_nl = []
        for channel in compsep_run["channels_out"]:
            if channel in good_channels_nl:
                output_maps_nl.append(output_maps_nl_[good_channels_nl.tolist().index(channel)])
            else:
                output_maps_nl.append(np.zeros((output_maps_nl_.shape[-2], output_maps_nl_.shape[-1])))
        del output_maps_nl_
        output_maps_nl = np.array(output_maps_nl)
    ...
    if compsep_run["method"] == "ilc":
        ...
        if "nlsquared" in compsep_run["compsep_path"]:
        ...
        output_alms_nl = np.array([
            np.array([
                hp.map2alm(output_maps_nl[i, :, c], lmax=lmax_, pol=False, **kwargs)
                for c in range(output_maps_nl.shape[-1])
            ]).T for i in range(output_maps_nl.shape[0])
        ])
        del output_maps_nl

        output_alms_j = np.zeros((output_alms_nl.shape[0], hp.Alm.getsize(config.lmax), output_alms_nl.shape[-1]), dtype=complex)
        for i in range(output_alms_nl.shape[0]):
            if "nlsquared" in compsep_run["compsep_path"]:
                output_alms_j[i] = _needlet_filtering(output_alms_nl[i], np.ones(lmax_+1), config.lmax)
            else:
                output_alms_j[i] = _needlet_filtering(output_alms_nl[i], b_ell[:lmax_+1], config.lmax)
...
def _combine_pixel(
    ...
    **kwargs: Any
    ...
    """
    Combine input multifrequency set of alms with component separation weights in pixel domain for a single scalar field.
    ...
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.
        input_alms : np.ndarray
            Input alms to be combined with pixel compsep weights for a single scalar field.
            The shape should be (n_freqs, n_alms, n_comps).
        compsep_run : dict
            Dictionary containing the residuals estimates run parameters. See '_combine_' for details.
        kwargs : dict
    ...
    -------
        np.ndarray
            Output maps resulting from combination with pixel component separation weights.
            Shape is (n_pixels, n_comps).
    """
    ...
    if w_.ndim==1:
        ...
        if compsep_run["method"] == "ilc":
            ...
            output_maps_ = np.einsum('li,ijk->ljk', w_, input_maps)
    elif w_.ndim==3:
        if compsep_run["method"] == "gilc":
            output_maps_ = np.einsum('jli,ijk->ljk', w_, input_maps)
    
    if compsep_run["method"] == "gilc":
        output_maps = []
        for channel in compsep_run["channels_out"]:
            if channel in good_channels:
                output_maps.append(output_maps_[good_channels.tolist().index(channel)])
            else:
                output_maps.append(np.zeros((output_maps_.shape[-2], output_maps_.shape[-1])))
        del output_maps_
        ...
        for c in range(output_maps.shape[-1]):
            if compsep_run["method"] == "ilc":
                ...
                for i in range(output_maps.shape[0]):
                    alm_out = hp.map2alm(output_maps[i,:,c],lmax=config.lmax, pol=False, **kwargs)
                    output_maps[i,:,c] = hp.alm2map(alm_out, config.nside, lmax=config.lmax, pol=False, pixwin=True)
...
def _combine_P_needlet(
    ...
    **kwargs: Any
    ...
    """
    Combines input multifrequency polarization alms with P(R)ILC component separation weights in needlet domain.
    ...
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.
        input_alms : np.ndarray
            Input alms for polarization field to be combined with needlet P(R)ILC weights.
            The shape should be (n_freqs, (n_fields), n_alms, n_comps), with n_fields not None if both E and B fields are present.
        compsep_run : dict
            Dictionary containing the residuals estimates run parameters. See '_combine_' for details.
        kwargs : dict
    ...
    -------
        np.ndarray
            Output maps resulting from propagation through P(R)ILC in needlet domain.
    ...
    for j in range(b_ell.shape[0]):
        output_maps += _combine_P_needlet_j(config, input_alms, compsep_run, b_ell[j], j, **kwargs)
...
#            else:
            ...
            for i in range(output_maps.shape[0]):
                for c in range(output_maps.shape[-1]):
                    alm_out = hp.map2alm(
                        [0.*output_maps[i,:,c], output_maps[i,0,:,c], output_maps[i,1,:,c]],
                        lmax=config.lmax, pol=True, **kwargs
                    )

                    if (config.field_out in ["QU", "QU_E", "QU_B"]):
                        output_maps[i,...,c] = hp.alm2map(alm_out, config.nside, lmax=config.lmax, pol=True, pixwin=True)[1:]
                    elif config.field_out in ["E","B"]:
                        output_maps[i,0,:,c] = hp.alm2map(alm_out[1] if config.field_out=="E" else alm_out[2], config.nside, lmax=config.lmax, pol=False, pixwin=config.pixel_window_out)
                    elif config.field_out=="EB":
                        output_maps[i,0,:,c] = hp.alm2map(alm_out[1], config.nside, lmax=config.lmax, pol=False, pixwin=config.pixel_window_out)
                        output_maps[i,1,:,c] = hp.alm2map(alm_out[2], config.nside, lmax=config.lmax, pol=False, pixwin=config.pixel_window_out)
    ...
    if "mask" in compsep_run:
        if compsep_run["method"] == "ilc":
            ...
            if output_maps.ndim == 4:
                output_maps[:,:,compsep_run["mask"] == 0.,:] = 0.
            elif output_maps.ndim == 3:
                output_maps[:,compsep_run["mask"] == 0.,:] = 0.

    return output_maps
        
def _combine_P_needlet_j(
    config: Configs,
    input_alms: np.ndarray,
    compsep_run: dict,
    b_ell: np.ndarray,
    ...
    **kwargs: Any
    ...
    """
    Combines input multifrequency polarization alms with P(R)ILC component separation weights for a single needlet band.
    ...
    -------
        np.ndarray
            Output maps resulting from combination with needlet P(R)ILC weights for a single band.
            Shape is (n_pixels, n_comps).
    """
    ...
    if w_pilc.ndim==1:
        ...
        if compsep_run["method"] == "ilc":
            if "prilc_" in compsep_run["compsep_path"]:
                output_maps_nl = np.einsum('ij,ifjk->fjk', w_pilc, input_maps_nl)
            elif "pilc_" in compsep_run["compsep_path"]:
            ...
            output_maps_nl_ = np.einsum('li,ifjk->lfjk', w_pilc, input_maps_nl)
    elif w_pilc.ndim==3:
        if compsep_run["method"] == "ilc":
            ...
            if "gprilc_" in compsep_run["compsep_path"]:
                output_maps_nl_ = np.einsum('jli,ifjk->lfjk', w_pilc, input_maps_nl)
            elif "gpilc_" in compsep_run["compsep_path"]:
                output_maps_nl_ = np.zeros((input_maps_nl.shape[0], 2, *input_maps_nl.shape[-2:]))
                output_maps_nl_[:, 0] = np.einsum('li,ijk->ljk', w_pilc[0], input_maps_nl[:,0]) - np.einsum('li,ijk->ljk', w_pilc[1], input_maps_nl[:,1])
                output_maps_nl_[:, 1] = np.einsum('li,ijk->ljk', w_pilc[1], input_maps_nl[:,0]) + np.einsum('li,ijk->ljk', w_pilc[0], input_maps_nl[:,1])
    elif w_pilc.ndim==4:
        if compsep_run["method"] == "gilc":
            output_maps_nl_ = np.zeros((input_maps_nl.shape[0], 2, *input_maps_nl.shape[-2:]))
            output_maps_nl_[:, 0] = np.einsum('jli,ijk->ljk', w_pilc[0], input_maps_nl[:,0]) - np.einsum('jli,ijk->ljk', w_pilc[1], input_maps_nl[:,1])
            output_maps_nl_[:, 1] = np.einsum('jli,ijk->ljk', w_pilc[1], input_maps_nl[:,0]) + np.einsum('jli,ijk->ljk', w_pilc[0], input_maps_nl[:,1])
    ...
    if compsep_run["method"] == "gilc":
        ...
        for channel in compsep_run["channels_out"]:
            if channel in good_channels_nl:
                ...
                output_maps_nl.append(np.zeros((2, output_maps_nl_.shape[-2], output_maps_nl_.shape[-1])))
        del output_maps_nl_
        output_maps_nl = np.array(output_maps_nl)
...
def _combine_P_pixel(config: Configs, input_alms, compsep_run, **kwargs):
    """
    Combines input multifrequency polarization alms with P(R)ILC component separation weights in pixel domain.
    ...
    ----------
        config : Configs
            Configuration object containing the global settings. See '_combine_' for details.
        input_alms : np.ndarray
            Input alms for polarization field to be combined with pixel P(R)ILC weights.
            The shape should be (n_freqs, (n_fields), n_alms, n_comps), with n_fields not None if both E and B fields are present.
        compsep_run : dict
            Dictionary containing the residuals estimates run parameters. See '_combine_' for details.
        kwargs : dict
    ...
    -------
        np.ndarray
            Output maps resulting from propagation through P(R)ILC in pixel domain.
            Shape is (n_fields, n_pixels, n_comps) where n_fields depends on config.field_out.
    """
    good_channels = _get_good_channels_nl(config, np.ones(config.lmax + 1))
    ...
    if w_pilc.ndim==1:
        ...
        if compsep_run["method"] == "ilc":
            ...
            output_maps_ = np.einsum('li,ifjk->lfjk', w_pilc, input_maps)
    elif w_pilc.ndim==3:
        if compsep_run["method"] == "ilc":
            ...
            if "gprilc_" in compsep_run["compsep_path"]:
                output_maps_ = np.einsum('jli,ifjk->lfjk', w_pilc, input_maps)
            elif "gpilc_" in compsep_run["compsep_path"]:
                output_maps_ = np.zeros((input_maps.shape[0], 2, *input_maps.shape[-2:]))
                output_maps_[:, 0] = (
                    np.einsum('li,ijk->ljk', w_pilc[0], input_maps[:, 0]) -
                    np.einsum('li,ijk->ljk', w_pilc[1], input_maps[:, 1])
                )
                output_maps_[:, 1] = (
                    np.einsum('li,ijk->ljk', w_pilc[1], input_maps[:, 0]) +
                    np.einsum('li,ijk->ljk', w_pilc[0], input_maps[:, 1])
                )
    elif w_pilc.ndim==4:
        if compsep_run["method"] == "gilc":
            output_maps_ = np.zeros((input_maps.shape[0], 2, *input_maps.shape[-2:]))
            output_maps_[:, 0] = (
                np.einsum('jli,ijk->ljk', w_pilc[0], input_maps[:, 0]) -
                np.einsum('jli,ijk->ljk', w_pilc[1], input_maps[:, 1])
            )
            output_maps_[:, 1] = (
                np.einsum('jli,ijk->ljk', w_pilc[1], input_maps[:, 0]) +
                np.einsum('jli,ijk->ljk', w_pilc[0], input_maps[:, 1])
            )
    ...
    if compsep_run["method"] == "gilc":
        ...
        for channel in compsep_run["channels_out"]:
            if channel in good_channels:
                ...
                output_maps.append(np.zeros((2, output_maps_.shape[-2], output_maps_.shape[-1])))
        del output_maps_
        output_maps = np.array(output_maps)
...
#            else:
            ...
            for i in range(output_maps.shape[0]):
                for c in range(output_maps.shape[-1]):
                    alm_out = hp.map2alm(
                        [0.*output_maps[i,0,:,c], output_maps[i,0,:,c], output_maps[i,1,:,c]],
                        lmax=config.lmax, pol=True, **kwargs
                    )
