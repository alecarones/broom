...
def merge_dicts(d: Union[List[Dict[Any, Any]], Dict[Any, Any]]) -> Dict[Any, Any]:
    ...
    Raises
    -------
        ValueError: If input is not a dict or list of single-key dicts.
...
def obj_to_array(obj: SimpleNamespace) -> np.ndarray:
    ...
    Parameters
    ----------
    obj: SimpleNamespace with attributes like "total", "fgds", etc.
...
def obj_out_to_array(obj: SimpleNamespace) -> np.ndarray:
    ...
    Parameters
    ----------
    obj: SimpleNamespace with attributes like "output_total", "noise_residuals", etc.
