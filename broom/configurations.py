...
#                setattr(self, attr, os.path.join(path_utils, getattr(self, attr)))
    ...
    def load_from_config(self, config_data: Dict[str, Any]):
        # Reset attributes before loading new config data
        self.reset_attributes()
        ...
        for attr in attrs:
            if attr in config_data:
                setattr(self, attr, config_data[attr])

        self.beams = config_data.get("beams", "gaussian")
        if self.beams != "gaussian":
            if 'path_beams' not in config_data:
                raise ValueError("Missing 'path_beams' for non-gaussian beams in the config data.")
            self.path_beams = config_data['path_beams']
        else:
            if 'fwhm' not in config_data:
                raise ValueError(f"FWHM must be provided in the config data for gaussian beams.")

        self.channels_tags = config_data.get("channels_tags", [])
        if not self.channels_tags:
            self._generate_channel_tags()
...
#        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        ...
        from .routines import get_prefix
        ...
        if self.lmin_in is None:
            self.lmin_in = self.lmin
...
#        self.path_utils = self.config.get("path_utils") or os.path.join(base_dir, "utils")
        ...
        foreground_models = self.config.get("foreground_models") or None
        if foreground_models is not None:
            prefix_models = ["d", "s", "co", "a", "f", "tsz", "cib", "ksz", "rg"]
            prefix_to_model = {}
            for model in foreground_models:
                prefix = get_prefix(model, prefix_models)
                if prefix is None:
                    raise ValueError(f"Unknown prefix in model: {model}")
                if prefix in prefix_to_model:
                    raise ValueError(f"Error: more models with prefix '{prefix}' in foreground_models.")
                prefix_to_model[prefix] = model
            self.foreground_models = [prefix_to_model[p] for p in prefix_models if p in prefix_to_model]
        else:
...
#        self.mask_type = self.config.get("mask_type") or "mask_for_compsep"
        ...
        if self.config.get("path_outputs"):
            self.path_outputs = self.config.get("path_outputs")
        else: 
        ...
        if (self.generate_input_data and self.save_inputs): # or (not self.generate_input_data):
            ...
            def_data_path = os.path.join(os.getcwd(), "inputs", self.experiment, "total")
            if self.foreground_models is not None:
                def_data_path = os.path.join(def_data_path, ''.join(self.foreground_models), dataname)
            else:
                def_data_path = os.path.join(def_data_path, dataname)
        else:
            def_data_path = None
...
#        self.cmb_path = os.path.normpath(os.path.join(os.getcwd(), self.cmb_path))
        ...
        if self.generate_input_foregrounds and self.save_inputs:
            ...
            def_fgds_path = os.path.join(os.getcwd(), "inputs", self.experiment, "foregrounds")
            if self.foreground_models is not None:
                def_fgds_path = os.path.join(def_fgds_path, ''.join(self.foreground_models), fgdsname)
            else:
                def_fgds_path = os.path.join(def_fgds_path, fgdsname)
        else:
            def_fgds_path = None
...
#        self.fgds_path = os.path.normpath(os.path.join(os.getcwd(), self.fgds_path))
        ...
        load_instrument = (self.config.get("instrument") is not None and isinstance(self.config.get("instrument"), Mapping)) or (self.config.get("instrument") is None and self.experiment)
...
#            raise ValueError("Path 'data_path' must be specified if 'generate_input_data' is False.")
    ...
    def _load_experiment_parameters(self):
        if self.config.get("instrument") is not None:
            self.instrument.load_from_config(self.config["instrument"])
        else:
