...
from ._ilcs import _get_good_channels_nl
from ._pilcs import get_pilc_cov, get_prilc_cov
from ._gilcs import _standardize_gnilc_run, Cn_C_Cn, _get_gilc_m
...
def gpilc(config: Configs, input_alms, compsep_run, **kwargs):
    if config.field_out not in ["E", "B", "QU", "EB", "QU_E", "QU_B"]:
        raise ValueError("Invalid field_out for GPILC. It must be E, B, QU, EB, QU_E, or QU_B.")
    ...
    output_maps = _gpilc(config, obj_to_array(input_alms), nuis_alms, compsep_run, **kwargs)
...
def fgd_P_diagnostic(config: Configs, input_alms, compsep_run, **kwargs):
    if config.field_in not in ["E", "B", "QU", "EB", "QU_E", "QU_B"]:
        raise ValueError("Invalid field_in for foreground diagnostic in P. It must be E, B, QU, EB, QU_E, or QU_B.")
    ...
    output_maps = _fgd_P_diagnostic(config, input_alms.total, nuis_alms, compsep_run)
...
def _gpilc(config: Configs, input_alms, nuis_alms, compsep_run, **kwargs):
    if compsep_run["domain"] == "pixel":
        output_maps = _gpilc_pixel(config, input_alms, nuis_alms, compsep_run, **kwargs)
    elif compsep_run["domain"] == "needlet":
        output_maps = _gpilc_needlet(config, input_alms, nuis_alms, compsep_run, **kwargs)
    return output_maps

def _fgd_P_diagnostic(config: Configs, input_alms, nuis_alms, compsep_run):
    if compsep_run["domain"]=="needlet":
        nls_number = _get_needlet_windows_(compsep_run["needlet_config"], config.lmax).shape[0]

    if compsep_run["domain"] == "pixel":
        output_maps = _fgd_P_diagnostic_pixel(config, input_alms, nuis_alms, compsep_run)
    elif compsep_run["domain"] == "needlet":
        output_maps = _fgd_P_diagnostic_needlet(config, input_alms, nuis_alms, compsep_run)

    return output_maps

def _gpilc_pixel(config: Configs, input_alms, nuis_alms, compsep_run, **kwargs):
    ...
    if ((config.field_out in ["QU", "QU_E", "QU_B"]) and config.pixel_window_out) or (config.field_out in ["E","B"]) or (config.field_out=="EB"):
        for f, c in np.ndindex(output_maps.shape[0],output_maps.shape[-1]):
            if ("mask" in compsep_run) and (config.mask_type == "observed_patch") and ("_purify" in config.leakage_correction):
                ...
                alm_out = hp.map2alm([0.*output_maps[f,0,:,c],output_maps[f,0,:,c],output_maps[f,1,:,c]],lmax=config.lmax, pol=True, **kwargs)
            ...
            if (config.field_out in ["QU", "QU_E", "QU_B"]):
                ...
                alm_out = hp.map2alm(np.array([0.*output_maps[f,0,:,c],output_maps[f,0,:,c],output_maps[f,1,:,c]]) * compsep_run["mask"],lmax=config.lmax, pol=True, **kwargs)
...
def _fgd_P_diagnostic_pixel(config: Configs, input_alms, nuis_alms, compsep_run):
    input_maps = np.zeros((input_alms.shape[0], 2, 12 * config.nside**2))
    nuis_maps = np.zeros((input_alms.shape[0], 2, 12 * config.nside**2))

    for n in range(input_alms.shape[0]):
        if input_alms.ndim == 4:
            nuis_maps[n] = hp.alm2map(np.array([0.*nuis_alms[n, 0],nuis_alms[n, 0],nuis_alms[n, 1]]), config.nside, lmax=config.lmax, pol=True)[1:]      
            input_maps[n] = hp.alm2map(np.array([0.*input_alms[n, 0],input_alms[n, 0],input_alms[n, 1]]), config.nside, lmax=config.lmax, pol=True)[1:]      
        elif input_alms.ndim == 3:
            if config.field_out in ["QU_E", "E"]:
                nuis_maps[n] = hp.alm2map(np.array([0.*nuis_alms[n],nuis_alms[n],0.*nuis_alms[n]]), config.nside, lmax=config.lmax, pol=True)[1:]
                input_maps[n] = hp.alm2map(np.array([0.*input_alms[n],input_alms[n],0.*input_alms[n]]), config.nside, lmax=config.lmax, pol=True)[1:]
            elif config.field_out in ["QU_B", "B"]:
                nuis_maps[n] = hp.alm2map(np.array([0.*nuis_alms[n],0.*nuis_alms[n],nuis_alms[n]]), config.nside, lmax=config.lmax, pol=True)[1:]
                input_maps[n] = hp.alm2map(np.array([0.*input_alms[n],0.*input_alms[n],input_alms[n]]), config.nside, lmax=config.lmax, pol=True)[1:]
                
    output_maps = _fgd_P_diagnostic_maps(config, input_maps, nuis_maps, compsep_run, np.ones(config.lmax+1))
...
def _gpilc_needlet(config: Configs, input_alms, nuis_alms, compsep_run, **kwargs):
    ...
    for j in range(b_ell.shape[0]):
        output_maps += _gpilc_needlet_j(config, input_alms, nuis_alms, compsep_run, b_ell[j], depro_cmb=(compsep_run["depro_cmb"])[j], m_bias=(compsep_run["m_bias"])[j], **kwargs)
    ...
    if ((config.field_out in ["QU", "QU_E", "QU_B"]) and config.pixel_window_out) or (config.field_out in ["E","B"]) or (config.field_out=="EB"):
        for f, c in np.ndindex(output_maps.shape[0],output_maps.shape[-1]):
            if ("mask" in compsep_run) and (config.mask_type == "observed_patch") and ("_purify" in config.leakage_correction):
                ...
                if "_iterations" in config.leakage_correction:
                    ialms_jterations = int(re.search(r'iterations(\d+)', config.leakage_correction).group(1))
                else:
                    iterations = 0
                alm_out = purify_recycling(output_maps[f,...,c], output_maps[f,...,0], compsep_run["mask"], config.lmax, purify_E=("E" in config.leakage_correction), iterations=iterations)
                alm_out = np.concatenate([(0.*alm_out[0])[np.newaxis],alm_out], axis=0)
...
def _gpilc_needlet_j(config: Configs, input_alms, nuis_alms, compsep_run, b_ell, depro_cmb=None, m_bias=0, **kwargs):
...
#    compsep_run["good_channels"] = np.arange(input_alms.shape[0])
    ...
    nuis_maps_nl = np.zeros((compsep_run["good_channels"].shape[0], 2, 12 * nside_**2))
...
def _fgd_P_diagnostic_needlet(config: Configs, input_alms, nuis_alms, compsep_run):
    b_ell = _get_needlet_windows_(compsep_run["needlet_config"], config.lmax)
    b_ell = b_ell**2
    ...
    for j in range(b_ell.shape[0]):
        output_maps[j] = _fgd_P_diagnostic_needlet_j(config, input_alms, nuis_alms, compsep_run, b_ell[j])
...
def _fgd_P_diagnostic_needlet_j(config: Configs, input_alms, nuis_alms, compsep_run, b_ell):
    if "mask" in compsep_run:
        nside_, lmax_ = config.nside, config.lmax
    else:
...
#    compsep_run["good_channels"] = _get_good_channels_nl(config, b_ell)
    ...
    for n, channel in enumerate(compsep_run["good_channels"]):
        ...
        if ("mask" in compsep_run) and (config.mask_type == "observed_patch"):
            nuis_maps_nl[n] = (hp.alm2map(np.array([0.*nuis_alms_j[0],nuis_alms_j[0],nuis_alms_j[1]]), nside_, lmax=lmax_, pol=True)[1:]) * compsep_run["mask"]       
            input_maps_nl[n] = (hp.alm2map(np.array([0.*input_alms_j[0],input_alms_j[0],input_alms_j[1]]), nside_, lmax=lmax_, pol=True)[1:]) * compsep_run["mask"]       
        else:
            nuis_maps_nl[n] = hp.alm2map(np.array([0.*nuis_alms_j[0],nuis_alms_j[0],nuis_alms_j[1]]), nside_, lmax=lmax_, pol=True)[1:]   
            input_maps_nl[n] = hp.alm2map(np.array([0.*input_alms_j[0],input_alms_j[0],input_alms_j[1]]), nside_, lmax=lmax_, pol=True)[1:]   

    output_maps_nl = _fgd_P_diagnostic_maps(config, input_maps_nl, nuis_maps_nl, compsep_run, b_ell)
    del input_maps_nl, nuis_maps_nl
    del compsep_run['good_channels']
...
def _fgd_P_diagnostic_maps(config: Configs, input_maps, nuis_maps, compsep_run, b_ell):
...
def _gpilc_needlet_old(config: Configs, input_alms, nuis_alms, compsep_run, **kwargs):
    b_ell = _get_needlet_windows_(compsep_run["needlet_config"], config.lmax)
    ...
    for j in range(b_ell.shape[0]):
        output_alms += _gpilc_needlet_j_old(config, input_alms, nuis_alms, compsep_run, b_ell[j], **kwargs)
    ...
    for n, channel in enumerate(good_channels_nl):
        ...
        for c in range(input_alms.shape[-1]):
            if ("mask" in compsep_run) and (config.mask_type == "observed_patch") and ("_purify" in config.leakage_correction):
                ...
                alm_out = hp.map2alm(np.concatenate([(0.*output_maps_nl[n,0,:,c])[np.newaxis],output_maps_nl[n,...,c]], axis=0), lmax=lmax_, pol=True, **kwargs)[1:]
